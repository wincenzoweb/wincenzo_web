...
const ReviewVideosList = ({ reviewVideos }) => {
  ...
  const COLUMNS = [
    {
      Header: "Video",
      accessor: "reviewVideoUrl",
      Cell: (row) => {
        return (
          <div>
            <div className="flex justify-end sm:justify-start lg:justify-end xl:justify-start -space-x-1 rtl:space-x-reverse">
              {/* {row?.cell?.data?.map((user, userIndex) => ( */}
              <div
                className="h-38 w-64 ring-1 ring-slate-100"
              // key={userIndex}
              >
                <video controls>
                  <source src={baseUrl + row?.cell?.value} type="video/mp4" />
    ...
    {
      Header: "Description",
      accessor: "reviewVideoDescription",
      Cell: (row) => {
        return (
          <div className="flex-1 font-medium text-sm leading-4 whitespace-nowrap">
            {row?.cell?.value?.length > 20
  ...
  const actions = [
    ...
    //   doit: (item) => navigate(`/admin/blog-details/${item._id}`),
    // },
    {
      name: "edit",
      icon: "heroicons:pencil-square",
      doit: (item) => dispatch(updateVideo(item)),
    },
    {
      name: "delete",
      ...
      doit: (item) => {
        ...
        dispatch(deleteReviewVideo(item._id));
  ...
  const data = useMemo(() => reviewVideos, [reviewVideos]);
    ...
    <>
      <Card noborder>
        <div className="md:flex justify-between items-center mb-6">
          <h4 className="card-title">Review Videos List</h4>
        </div>
...
export default ReviewVideosList;
