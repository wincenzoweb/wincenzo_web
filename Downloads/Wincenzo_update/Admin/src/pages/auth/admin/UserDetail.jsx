...
const UserDetail = () => {
  ...
  const detail = assignments?.filter((el) => {
    return el?.deliveryBoyId?._id === detailItem?._id;
  });
...
const TableContent = ({ detail }) => {
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      accessor: "orderId.paymentMethod",
      Cell: (row) => {
        return <span>{row?.cell?.value}</span>;
      },
    },
      ...
      Cell: (row) => {
        return (
          <span className="block w-full">
            ...
            ${
              row?.cell?.value !== "return" || row.cell.value !== "delivered"
                ? "text-warning-500 bg-warning-500"
                : ""
            }
            ${
