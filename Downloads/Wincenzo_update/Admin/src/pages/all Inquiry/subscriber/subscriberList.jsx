...
import { deleteSubscriber, getSubscriber, updateData } from "../../../store/features/inquiry";
...
const SubscriberList = ({ subscriber }) => {
    ...
    },
    {
      ...
      Cell: (row) => {
        return (
          <div>
            <Dropdown
              classMenuItems="right-0 w-[140px] top-[-40px] "
              label={
  ...
  const actions = [
    ...
    //   doit: (item) => navigate(`/admin/categories/${item._id}`),
    // },
    // {
    //   name: "edit",
    ...
    // },
    {
      ...
      doit: (item) => {
        ...
        dispatch(deleteSubscriber(item._id));
  ...
  const data = useMemo(() => subscriber, [subscriber]);

  const handleExport = () => {
    const dataToExport = subscriber.map(subscriberItem => ({
      "Email": subscriberItem.email,
      "Date": dayjs(subscriberItem.createdAt).format("DD/MM/YYYY"),
    }));
    ...
    <>
      <Card noborder>
        <div className="flex justify-between items-center mb-6">
          <h4 className="card-title">Subscriber List</h4>
          <div className={`${width < breakpoints.md ? "space-x-rb" : ""
            } flex md:space-x-4 sm:justify-end items-center rtl:space-x-reverse `}>
            <CSVLink
              data={csvData}
