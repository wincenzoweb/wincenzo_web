...
import { deleteInquiry, getInquiry, updateData } from "../../../store/features/inquiry";
...
const InquiryList = ({ inquiry }) => {
    ...
    },
    {
      ...
      Cell: (row) => {
        return (
          <div>
            <Dropdown
              classMenuItems="right-0 w-[140px] top-[-75px] "
              label={
  ...
  const actions = [
    ...
    },
    {
      ...
      doit: (item) => {
        ...
        dispatch(deleteInquiry(item._id));
  ...
  const handleExport = () => {
    const dataToExport = inquiry.map(inquiryItem => ({
      ...
      "Date": dayjs(inquiryItem.createdAt).format("DD/MM/YYYY"),
    }));
  ...
  const data = useMemo(() => inquiry, [inquiry]);
